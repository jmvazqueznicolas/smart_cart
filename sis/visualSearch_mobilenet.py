...
def main():
    ...
    for feature_path in Path("./static/feature/mobilenet").glob("*.npy"):
    ...
    while cap.isOpened():
        ...
        """
        if ((scores[0][0] < 0.81) or ((product_name0 == product_name1) and (product_name0   ==  product_name2))): 
            image_np = cv2.putText(image_np, product_name0, org, font, fontScale, color,        thickness, cv2.LINE_AA)
        """
        if (scores[0][0] < 0.81):
            image_np = cv2.putText(image_np, product_name0, org, font, fontScale, color,    thickness, cv2.LINE_AA)

        elif ((product_name0 == product_name1) or (product_name0 ==  product_name2)):
            image_np = cv2.putText(image_np, product_name0, org, font, fontScale, color,    thickness, cv2.LINE_AA)

        elif ((product_name1 == product_name0) or (product_name1 ==  product_name2)):
            image_np = cv2.putText(image_np, product_name1, org, font, fontScale, color,    thickness, cv2.LINE_AA)

        elif ((product_name2 == product_name0) or (product_name2 ==  product_name1)):
            image_np = cv2.putText(image_np, product_name2, org, font, fontScale, color,    thickness, cv2.LINE_AA)
